    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">Enquiry Mode</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ url('/admin') }}">Dashboard</a>
                        </li>
                        <li class="breadcrumb-item active">Enquiry Mode</li>
                    </ul>
                </div>
                @if (in_array('12', $add_perm) || in_array('12', $delete_perm))
                    <div class="col-auto">
                        @if (in_array('12', $add_perm))
                        <a class="btn btn-primary me-1" href="{{ route('enquiry_mode.create') }}">
                            <i class="fas fa-plus"></i> Add Enquiry Mode
                        </a>
                        @endif
                        @if (in_array('12', $delete_perm))
                        <a class="btn btn-danger me-1" href="javascript:void('0');" onclick="delete_enquiry_mode();">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="card card-table">
                    <div class="card-body">
                        <form id="form" action="{{ route('delete_enquiry_mode') }}" enctype="multipart/form-data">
                                                ...
                                                @if (in_array('12', $edit_perm))
                                                    <td class="text-right">
                                                        <a class="btn btn-primary"
                                                            href="{{ route('enquiry_mode.edit', $data->id) }}"><i
...
@section('footer_js')
    ...
    <div class="modal custom-modal fade" id="delete_enquiry_mode" role="dialog">
 ...
 <div class="modal-dialog modal-dialog-centered">
    ...
    <!-- /set orderModal -->
    <script>
        function delete_enquiry_mode() {
            // alert('test');
            var checked = $("#form input:checked").length > 0;
            if (!checked) {
                $('#select_one_record').modal('show');
            } else {
                $('#delete_enquiry_mode').modal('show');
            }
        }
        function form_sub() {
            $('#form').submit();
